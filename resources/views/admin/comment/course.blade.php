...
@section('breadcrumb')
    <ul class="page-breadcrumb">
        ...
        <li>
            <a href="{{ url('admin/comment/course') }}">Đánh giá</a>
        </li>
    </ul>
@endsection
...
@section('content')
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            {!! Form::close() !!}
            <select name="" id=""  class="form-control input-small input-inline pull-right" onchange="location = this.value;">
                ...
                <div class="portlet-body flip-scroll">
                    <table class="table table-bordered table-striped table-condensed flip-content">
                        ...
                        @foreach ($list as $key => $item)
                            <tr>
                                <td> {{ $key + 1 }} </td>
                                <td><a href="{{$item->url}}">{!!$item->course->title  !!}</a></td>
                                <td> {{ $item->name }} </td>
                                <td> {{ $item->email }} </td>
                                <td> {{ $item->content }} </td>
                            </tr>
