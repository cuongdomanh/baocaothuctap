<div class="form-body">
    ...
    <div class="form-group {{ $errors->has('thumbnai') ? 'has-error' : ''}}">
        <label class="control-label col-md-3">Ảnh đại diện </label>
        <div class="col-md-4">
            {!! Form::file('thumbnai', null, ['class' => 'form-control', 'data-required' => '1', 'files'=>true, 'method' =>'post', 'url' => 'admin/user']) !!}
            {!! $errors->first('thumbnai', '<span id="name-error" class="help-block help-block-error">:message</span>') !!}
        </div>
    </div>

    <div class="form-group {{ $errors->has('info') ? 'has-error' : ''}}">
        <label class="control-label col-md-3">Thông tin giảng viên</label>
        <div class="col-md-8">
            {!! Form::textarea('info', null, ['class' => 'form-control ckeditor ']) !!}
            {!! $errors->first('info', '<span id="info-error" class="help-block help-block-error">:message</span>') !!}
        </div>
    </div>

    <div class="form-group">
        <label class="control-label col-md-3">Kích hoạt<span class="required"> * </span></label>
        <div class="col-md-4">
            {!! Form::checkbox('status', '1', (isset($user->status) && $user->status == 1) ? true : false, ['class' => 'form-control']) !!}
        </div>
    </div>


    <div class="form-group">
        <label class="control-label col-md-3">Roles <span class="required"> * </span></label>
        <div class="col-md-4">
